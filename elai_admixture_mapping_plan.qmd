...
**Symbol key:** $S$ upper clusters (sources), $K$ lower clusters (haplotype clusters),
$\theta_{mk}$ cluster allele frequencies, $\alpha_s^{(i)}$ admixture proportions,
$\beta_{msk}$ lower-given-upper probabilities, $j_m$ upper no-switch / $r_m$ lower switch
probabilities, $\gamma$ admixture generations, $\lambda$ mean tract length (cM), $\mu$ error
term, $c_m$ inter-marker genetic distance.

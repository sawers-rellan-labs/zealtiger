...
Order-of-magnitude for the current run: `100 × 50,000 × (3·10)² × 2 × ~tens of
EM iterations` ≈ a few ×10¹¹ operations on one core → ~10+ minutes at r=10.
...
| Fixed (current) | 100 | `autotune=FALSE`, **r=10** | the fast, single-fit configuration |

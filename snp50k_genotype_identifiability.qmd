---
title: "Why the SNP50K skim cannot resolve zygosity — two mechanisms, with the math"
...
among 73.4M cells (see [snp50k-cohort-provenance.md](https://github.com/sawers-rellan-labs/zealtiger/blob/main/docs/snp50k-cohort-provenance.md))
...
> [genotype-likelihoods-and-hmm.md](genotype_likelihoods_and_hmm.html).
...
[genotype-likelihoods-and-hmm.md](genotype_likelihoods_and_hmm.html) §2.
...
1. Grzybowski MW, Mural RV, Xu G, Turkus J, Yang J, Schnable JC (2023). A common
   resequencing-based genetic marker data set for global maize diversity.
   *The Plant Journal* **113**(6):1109–1121.
   [doi:10.1111/tpj.16123](https://doi.org/10.1111/tpj.16123).
   — the teosinte reference panel used to estimate the donor dose (§2).
2. Li H (2011). A statistical framework for SNP calling, mutation discovery,
   ...
   — the samtools/bcftools caller and GL model behind the `call -mv` prior (§1).
3. Hui R, D’Atanasio E, Cassidy LM, Scheib CL, Kivisild T (2020). Evaluating
   ...
   *Scientific Reports* **10**:18542.
   ...
   — reference-panel (aDNA) imputation that does not transfer here (§2.6).
4. Jensen SE, Charles JR, Muleta K, *et al.* (2020). A sorghum practical haplotype
   ...
   *The Plant Genome* **13**(1):e20009.
   ...
   — inbred-founder PHG imputation that does not transfer here (§2.6).

Full caller / genotype-likelihood method-paper references (GATK HaplotypeCaller,
ANGSD, Li–Stephens) are in the companion
[genotype_likelihoods_and_hmm.html](genotype_likelihoods_and_hmm.html#references).

------
*Analysis 2026-07-02, `cohort.vcf.gz` / `bzea_50K_ref_panel.vcf.gz` (read-only mount).
Companion to [snp50k-cohort-provenance.md](https://github.com/sawers-rellan-labs/zealtiger/blob/main/docs/snp50k-cohort-provenance.md). Panel:
